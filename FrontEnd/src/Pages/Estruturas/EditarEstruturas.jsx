import css from "./EditarEstruturas.module.css";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm } from "react-hook-form";
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { EditarEstruturasModal } from "../../Components/Modais/Estruturas/EditarEstruturasModal";
import axios from "axios";

const validacaoEditarEstrutura = z.object({
    nome: z.string()
...
export function EditarEstruturas() {
    const [editarEstruturaModal, setEditarEstruturaModal] = useState(false);
    ...
    const {
        ...
        resolver: zodResolver(validacaoEditarEstrutura)
    });

    async function put_estruturas(data) {
        ...
        try {
            await axios.put(`http://127.0.0.1:8000/MineLucas/estruturas/${idEstrutura}/`, dadosEstrutura, {
                headers: {
            ...
            setEditarEstruturaModal(true);
        }
        catch(error) {
            console.error("Erro ao atualizar estrutura: ", error.response?.data || error.message);
        }
    }
    ...
    return (
        <main style={{ backgroundColor:'rgba(0, 0, 0, 0.5)', backgroundBlendMode:'darken' }}>
            <section className={css.formularioEstruturas}>
                <h1>Edite sua estrutura aqui.</h1>
                <p className={css.aviso}>Verifique as informações antes de enviar.</p>
                <form onSubmit={handleSubmit(put_estruturas)}>
                    <label 
                        htmlFor="nome"
                        style={{ fontSize:"20px" }}>
                        Nome:
                    ...
                    <div className={css.botoes}>
                        ...
                        <EditarEstruturasModal openModal={editarEstruturaModal}/>            
                    </div>
                </form>
            </section>
        </main>
