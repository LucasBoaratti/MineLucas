import css from "./CriarJogadores.module.css";
import { useNavigate } from "react-router-dom";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { useForm } from "react-hook-form";
import { useState } from "react";
import { CriarJogadoresModal } from "../../Components/Modais/Jogadores/CriarJogadoresModal";
import axios from "axios";

const validacaoCriarJogador = z.object({
    nome: z.string()
...
export function CriarJogadores() {
    const [criarJogadorModal, setCriarJogadorModal] = useState(false);
    ...
    const {
        ...
        resolver: zodResolver(validacaoCriarJogador)
    });

    async function post_jogadores(data) {
        const dadosJogador = {
            ...data,
        }
        ...
        try {
            await axios.post("http://127.0.0.1:8000/MineLucas/jogadores/", dadosJogador, {
                headers: {
            ...
            setCriarJogadorModal(true);
        }
        catch(error) {
            console.error("Erro ao criar jogador: ", error.response?.data || error.message);
        }
    }
    ...
    return (
        <main style={{ backgroundColor:'rgba(0, 0, 0, 0.5)', backgroundBlendMode:'darken' }}>
            <section className={css.formularioJogadores}>
                <h1>Crie seu jogador aqui!</h1>
                <p className={css.aviso}>Pode ser um jogador existente ou fictício.</p>
                <form onSubmit={handleSubmit(post_jogadores)}>
                    <label 
                        htmlFor="nome"
                        style={{ fontSize:"20px" }}>
                        Nome:
                    ...
                    <div className={css.botoes}>
                        ...
                        <CriarJogadoresModal openModal={criarJogadorModal}/>
                    </div>
                </form>
            </section>
        </main>
